...
### 2026-04-24 知识图谱数据链路修复 + 回填

**Bug 修复（第三轮）：**

- **BUG-20240424-001**：修复 `userAnswersApi.add()` 调用时缺少 `await` 导致知识图谱数据从未成功保存至数据库的问题。改为 `await` 并纳入主流程错误处理，保存失败时用户可见错误提示（`Quiz.vue`）
- **BUG-20240424-002**：移除知识图谱区块外层 `v-if="hasRadarData"` 条件渲染，改为始终显示区块、仅内部图表按数据切换空/有数据状态，避免页面布局跳动（`Analytics.vue`）

**历史数据回填：**

- 新增 `backfillUserAnswers.js` 脚本：将 `practice_history` 中的历史练习记录按分类均摊估算，填充 `user_answers` 表，使历史数据能在知识图谱中展示
- 运行方式：`node src/scripts/backfillUserAnswers.js`
- 注意：估算数据仅用于历史展示，不代表真实每题对错
